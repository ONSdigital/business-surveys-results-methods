...
def dataframe_to_string(
    ...
    first_row[0] = f"{first_row[0]}"
    other_rows = [[f"{str(item)}," for item in row] for row in df.to_numpy()]
    ...
    data_string = data_string.replace("nan,", "None,")
    data_string = data_string.replace("nan)", "None)")
    data_string = f"df = create_test_dataframe([{data_string}])\nreturn df\n\n"
    ...
    # data_string = f"return create_test_dataframe([{data_string}])\n"
...
def generate_test_code(
    ...
    imports = (
        ...
        "import numpy as np\n"
        "import pytest\n"
        "from iabs.utils.helpers import create_test_dataframe\n"
...
def process_dataframe(
    ...
    exp_output_dfs: dict[str, pd.DataFrame],
    output_directory: str,
    function_name: str,
    module_name: str,
    column_type_override: dict[str, list[str]] | None = None,
    ...
    for file_name, df in input_dfs.items():
        input_strings[file_name] = dataframe_to_string(
            df, file_name, column_type_override, padding=padding
        )
        logging.info(f"Successfully processed input: {file_name}")

    for file_name, df in exp_output_dfs.items():
        output_strings[file_name] = dataframe_to_string(
            df, file_name, column_type_override, padding=padding
        )
        logging.info(f"Successfully processed expected output: {file_name}")
...
if __name__ == "__main__":
    # COPY AND PASTE DATAFRAMES HERE
    #     You can copy and paste the Python code that defines dfs in old-style unit tests
    #     here, then add them to the input_dfs and exp_output_dfs dictionaries below
    sample_question_columns = ["sic92", "ruref", "question", "value"]
    sample_question_data = [
        ["56101", "12345678901", "9", 10.0],
        ["56101", "12345678901", "10", 10123.0],
        ["56101", "12345678901", "11", 14.0],
        ["56302", "98765432109", "9", 2.0],
        ["56302", "98765432109", "15", 1.0],
    ]
    sample_question_df = pd.DataFrame(sample_question_data, columns=sample_question_columns)

    expected_pivot_columns = ["sic92", "ruref", "Q9", "Q10", "Q11", "Q15"]
    expected_pivot_data = [
        ["56101", "12345678901", 10.0, 10123.0, 14.0, None],
        ["56302", "98765432109", 2.0, None, None, 1.0],
    ]
    expected_pivot_df = pd.DataFrame(expected_pivot_data, columns=expected_pivot_columns)

    # cases = [
    #     {
    #         "case_name": "pivot_question_dataframe_happy_path"
    #         "function_name": "pivot_question_dataframe",
    #         "module_name": "iabs.staging.staging_utils",
    #         "input_dfs": {
    #             "sample_question_dataframe.csv": sample_question_df,
    #         },
    #         "expected_dfs": {
    #             "expected_pivoted_question_dataframe": expected_pivot_df,
    #         },
    #     }
    # ]
    config = TestConfig(
        input_dfs={"sample_question_df": sample_question_df},
        exp_output_dfs={"expected_pivot_df": expected_pivot_df},
        output_directory="D:/iabs-results-processing/tests/",
