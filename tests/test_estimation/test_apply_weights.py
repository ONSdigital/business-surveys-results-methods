...
def input_df():
    ...
    data = [
        ["A", 1, 10.0, 2.0, 40.0 / 12.0],
        ["B", 1, 20.0, 2.0, 40.0 / 12.0],
        ["C", 1, 30.0, 2.0, 40.0 / 12.0],
        ["D", 2, 15.0, 2.0, 1.0],
        ["E", 2, 25.0, 2.0, 1.0],
    ]
...
def expected_a_and_g_df():
    ...
    for ruref A (cell_no=1):
       input:    y=10.0, a_weight=2.0, g_weight=40.0/12.0=3.3333
       y_new = y * a_weight * g_weight
              = 10.0  * 2.0      * 3.3333
              = 66.6667 (rounded to 4dp)
    """
    cols = ["ruref", "cell_no", "y", "a_weight", "g_weight"]
    data = [
        ["A", 1, 66.6667, 2.0, 40.0 / 12.0],
        ["B", 1, 133.3333, 2.0, 40.0 / 12.0],
        ["C", 1, 200.0, 2.0, 40.0 / 12.0],
        ["D", 2, 30.0, 2.0, 1.0],
        ["E", 2, 50.0, 2.0, 1.0],
    ]
    return pd.DataFrame(data=data, columns=cols)
...
def expected_a_only_df():
    ...
    data = [
        ["A", 1, 20.0, 2.0, 40.0 / 12.0],
        ["B", 1, 40.0, 2.0, 40.0 / 12.0],
        ["C", 1, 60.0, 2.0, 40.0 / 12.0],
        ["D", 2, 30.0, 2.0, 1.0],
        ["E", 2, 50.0, 2.0, 1.0],
    ]
    return pd.DataFrame(data=data, columns=cols)
...
def test_apply_a_and_g_weights(input_df, expected_a_and_g_df):
    ...
    assert_frame_equal(result, expected_a_and_g_df, rtol=1e-6)
...
def test_apply_a_weight_only(input_df, expected_a_only_df):
    ...
    assert_frame_equal(result, expected_a_only_df, rtol=1e-6)
